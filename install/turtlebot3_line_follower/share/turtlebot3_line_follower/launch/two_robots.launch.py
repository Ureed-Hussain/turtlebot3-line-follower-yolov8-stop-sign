...
def robot_group(ns: str,            # namespace & Gazebo entity name
    ...
    return GroupAction([
        ...
        Node(  # depth watchdog
            package="turtlebot3_line_follower",
            executable="two_robot_depth",
            name="two_robot_depth",
            output="screen"),
        Node(
            package='turtlebot3_line_follower',
            executable='two_robot_camera',
            name='two_robot_camera',
            output='screen',
            parameters=[
                {'camera_threshold': 1}  # Minimum number of circles to trigger stop
            ]
        ),
    ])
